    ...
    public class AccountDataStore : IAccountDataStore
    {
       ...
       bool IAccountDataStore.DeleteAccount(string accountNumber)
        ...
        Account IAccountDataStore.GetAccount(string accountNumber)
        {
            using (IDbConnection db = getConnection())
            {
                return db.Query<Account>("SELECT [AccountNumber] ,[Balance],[Status],[AllowedPaymentSchemes] FROM [Accounts] WHERE [AccountNumber] =@AccountNumber", new { AccountNumber = accountNumber }).SingleOrDefault();
            }
        }
        ...
        List<Account> IAccountDataStore.GetAccounts(int rowCount, string sort)
        {
            using (IDbConnection db = getConnection())
            {
                return db.Query<Account>("SELECT TOP " + rowCount.ToString() + " [AccountNumber] ,[Balance],[Status],[AllowedPaymentSchemes]  FROM [Accounts] ORDER BY  " + sort).ToList();
            }
        }
